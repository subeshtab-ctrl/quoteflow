...
export const OrganizationSettingsSchema = z.object({
  ...
  business_type: z.string().nullish().or(z.literal('')),
  email: z.string().nullish().or(z.literal('')),
  phone: z.string().nullish().or(z.literal('')),
  website: z.string().nullish().or(z.literal('')),
  gst_vat_number: z.string().nullish().or(z.literal('')),
  address_line1: z.string().nullish().or(z.literal('')),
  address_line2: z.string().nullish().or(z.literal('')),
  city: z.string().nullish().or(z.literal('')),
  state: z.string().nullish().or(z.literal('')),
  country: z.string().nullish().or(z.literal('')).default('India'),
  postal_code: z.string().nullish().or(z.literal('')),
  logo_url: z.string().nullish().or(z.literal('')),
  brand_color: z.string().nullish().or(z.literal('')).default('#4f46e5'),
  default_currency: z.enum(['INR', 'USD', 'AED', 'EUR', 'GBP']).default('INR'),
  default_tax_rate: z.coerce.number().min(0).max(100).default(18),
  default_validity_days: z.coerce.number().int().default(30),
  quotation_prefix: z.string().default('Q-'),
